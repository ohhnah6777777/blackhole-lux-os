...
function Browser({ proxyUrl }: { proxyUrl: string }) {
  ...
  const closeTab = (id: number) => {
    ...
    const next = tabs.filter((tab) => tab.id !== id); setTabs(next); if (id === activeId) setActiveId(next[next.length - 1].id);
  };
...
function Files() {
  const [items, setItems] = useState<FsItem[]>(() => ["Documents", "Downloads", "Pictures", "Games"].map((name, index) => ({ id: index + 1, name, type: "folder", parent: "Home" })).concat([{ id: 10, name: "Welcome.txt", type: "file", parent: "Documents", content: "Welcome to Blackhole OS.\n\nThis file is yours to edit." }]));
